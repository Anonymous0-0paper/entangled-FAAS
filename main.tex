...
Evaluating a unified quantum-classical serverless model requires bridging the execution limits of classical cloud functions with those of quantum hardware. Because deep co-scheduling access is restricted on current public quantum clouds, we developed a discrete-event simulator using SimPy. 
The simulator replays historical QPU execution logs and queue trace data obtained from the IBM Quantum Network, superimposed with classical function invocation profiles modeled after AWS Lambda. This allows us to precisely track $t_{queue}$, $t_{cpu}$, and $t_{qpu}$ under varying workload densities. The specific hardware and software configurations used for the simulation testbed are detailed in Table \ref{tab:testbed}.
...
Classical Orchestrator & Kubernetes v1.28 \\
FaaS Environment & OpenFaaS (1024MB RAM) \\
Quantum SDK & Qiskit  \\
Quantum Simulator & Qiskit Aer  \\
Trace Data Source & IBM Quantum (Eagle r3, 127-qubit) \\
Python Version & 3.11.5 \\ \hline
\end{tabular}
\end{table}

\textbf{Workload and Hyperparameters.}
Our primary benchmark is a VQE algorithm that computes the ground-state energy of a Lithium Hydride (LiH) molecule. The classical optimizer uses Simultaneous Perturbation Stochastic Approximation (SPSA), which is highly robust to quantum noise but requires a large number of iterations (typically $>500$). 

We benchmark Entangled-FaaS against three architectural baselines currently utilized in quantum-cloud ecosystems:
\begin{enumerate}
...
Ansatz Circuit & \texttt{RealAmplitudes} (Depth 4) \\
Optimizer & SPSA (Max Iterations: 1000) \\
Shots per Evaluation & 4096 \\
Drift Threshold ($\tau_{drift}$) & 300 seconds \\
Session Boost Weight ($\alpha$) & 100.0 \\
Wait Time Weight ($\beta$) & 5.0 \\
Fair-Share Weight ($\gamma$) & 1.0 \\ \hline
\end{tabular}
\end{table}
